...
static bool ggml_cuda_should_fuse_mul_mat_vec_q(const ggml_tensor * tensor) {
    ...
    bool use_mul_mat_vec_q = ggml_is_quantized(src0->type) && !bad_padding_clear && !is_tq_weight && src1->type == GGML_TYPE_F32 &&
...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->op == GGML_OP_MUL_MAT_ID) {
            ...
            const bool is_tq_weight = node->src[0]->type == GGML_TYPE_TQ4_1S || node->src[0]->type == GGML_TYPE_TQ3_1S;
            if (!ggml_is_quantized(node->src[0]->type) || is_tq_weight || node->ne[2] > mmvq_mmid_max) {
