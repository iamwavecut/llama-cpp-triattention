...
void quantize_row_tq3_1s_ref(const float * GGML_RESTRICT x, block_tq3_1s * GGML_RESTRICT y, int64_t k) {
    ...
    for (int block = 0; block < nb; block++) {
        ...
        for (int group = 0; group < 4; group++) {
            uint32_t packed = 0;
            for (int j = 0; j < 8; j++) {
                int idx_el = group * 8 + j;
                float inv = idx_el < 16 ? inv0 : inv1;
                int idx = tq3_0_choose_index(buf[idx_el] * inv);
                packed |= (uint32_t) (idx & 0x7) << (3 * j);
            }
            blk->qs[group * 3 + 0] = packed & 0xFF;
            blk->qs[group * 3 + 1] = (packed >> 8) & 0xFF;
            blk->qs[group * 3 + 2] = (packed >> 16) & 0xFF;
        }
    }
}
    ...
    for (int blk_i = 0; blk_i < nb; blk_i++) {
        ...
        for (int group = 0; group < 4; group++) {
            const uint32_t packed =
                ((uint32_t) x[blk_i].qs[group * 3 + 0]) |
                ((uint32_t) x[blk_i].qs[group * 3 + 1] << 8) |
                ((uint32_t) x[blk_i].qs[group * 3 + 2] << 16);
            for (int j = 0; j < 8; j++) {
                int idx_el = group * 8 + j;
                uint8_t idx = (packed >> (3 * j)) & 0x7;
                float d = idx_el < 16 ? d0 : d1;
                buf[idx_el] = TQ3_0_CENTROIDS[idx] * d;
            }
        }
